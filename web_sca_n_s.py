...
# Constants for terminal manipulation
...
def check_internet():
    os.system('ping -c1 github.com > Web_Sca_N_S_net 2>&1')
    if "0% packet loss" in open('Web_Sca_N_S_net').read():
        val = 1
    else:
        val = 0
    os.system('rm Web_Sca_N_S_net > /dev/null 2>&1')
    return val
...
# Function to provide information about vulnerability
...
# Web_Sca_N_S Help Context
def helper():
    print(bcolors.OKBLUE + "Information:" + bcolors.ENDC)
    print("------------")
    print("\t./Web_Sca_N_S.py example.com: Scans the domain example.com.")
    print("\t./Web_Sca_N_S.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
    print("\t./Web_Sca_N_S.py example.com --nospinner: Disable the idle loader/spinner.")
    print("\t./Web_Sca_N_S.py --update   : Updates the scanner to the latest version.")
    print("\t./Web_Sca_N_S.py --help     : Displays this help context.")
    print(bcolors.OKBLUE + "Interactive:" + bcolors.ENDC)
    print("------------")
    print("\tCtrl+C: Skips current test.")
    print("\tCtrl+Z: Quits Web_Sca_N_S.")
...
# Function to clear line in terminal
def clear():
    sys.stdout.write("\033[F")
    sys.stdout.write("\033[K")  # clears until EOL

# Initializing the custom spinner class
class WebScaNSpinner:
    busy = False
    delay = 0.005  # 0.05
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t" + bcolors.BG_ERR_TXT + "Web_Sca_N_S received a series of Ctrl+C hits. Quitting..." + bcolors.ENDC)
            sys.exit(1)
...
if args_namespace.help or (not args_namespace.update \
    ...
    print("Web_Sca_N_S is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum Web_Sca_N_S.py | grep .... | cut -c 1-40'
    oldversion_hash = subprocess.check_output(cmd, shell=True)
    oldversion_hash = oldversion_hash.strip()
    os.system('wget -N https://raw.githubusercontent.com/Web_Sca_N_S/master/Web_Sca_N_S.py -O Web_Sca_N_S.py > /dev/null 2>&1')
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t" + bcolors.OKBLUE + "You already have the latest version of Web_Sca_N_S." + bcolors.ENDC)
    else:
        clear()
        print("\t" + bcolors.OKGREEN + "Web_Sca_N_S successfully updated to the latest version." + bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    os.system('rm /tmp/Web_Sca_N_S* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t" + bcolors.BG_ERR_TXT + "Web_Sca_N_S was terminated abruptly..." + bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t" + bcolors.OKGREEN + "All Scanning Tools are available. Complete vulnerability checks will be performed by Web_Sca_N_S." + bcolors.ENDC)
    else:
        print("\t" + bcolors.WARNING + "Some of these tools " + bcolors.BADFAIL + str(unavail_tools_names) + bcolors.ENDC + bcolors.WARNING + " are unavailable or will be skipped. Web_Sca_N_S will still perform the rest of the tests. Install these tools to fully utilize the functionality of Web_Sca_N_S." + bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/Web_Sca_N_S_temp_" + tool_names[tool][arg1]
        ...
        if runTest == 1:
            ...
            print(bcolors.OKBLUE + "\nScan Completed in " + display_time(int(elapsed)) + bcolors.ENDC + "\n", end='\r', flush=True)
            print("\n")
            ...
            print("\n" + bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit Web_Sca_N_S.\n" + bcolors.ENDC)
            rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list) == 0:
        ...
        with open(vulreport, "a") as report:
            while rs_vul < len(rs_vul_list):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/Web_Sca_N_S_temp_" + vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for " + bcolors.OKBLUE + target + bcolors.ENDC + " named " + bcolors.OKGREEN + vulreport + bcolors.ENDC + " is available under the same directory Web_Sca_N_S resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open(debuglog, "a") as report:
            try:
                with open("/tmp/Web_Sca_N_S_temp_" + file_name[arg1], 'r') as temp_report:
                    data = temp_report.read()
    ...
    os.system('rm /tmp/Web_Sca_N_S_te* > /dev/null 2>&1') # Clearing previous scan files